...
class TtReduceModule(LightweightModule):
    ...
    def forward(
        ...
        if weights is not None:
            ...
            # topk=1 is ambiguous if we only inspect weights.shape[-1], so also check
            # whether the penultimate dim is the combine expert/top-k dimension.
            combine_topk = combine_output.shape[self.topk_dim]
            has_trailing_broadcast_dim = (
                len(weights.shape) >= 2 and weights.shape[-1] == 1 and weights.shape[-2] == combine_topk
            )
            if not has_trailing_broadcast_dim:
                weights = ttnn.unsqueeze(weights, dim=-1)

            # Add leading batch/dispatch dimensions if needed to match combine_output rank.
