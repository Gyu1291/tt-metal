...
class TtMoERoutingSetup(LightweightModule):
    ...
    def __init__(
        ...
        self.last_host_dispatch_offsets: torch.Tensor | None = None
        self.last_host_total_counts: torch.Tensor | None = None
    ...
    def _single_device_offset_cumsum(
        self,
        expert_histograms: ttnn.Tensor,
        host_expert_histogram: torch.Tensor | None = None,
    ):
        """Compute offset_cumsum outputs without CCL for a 1x1 opt run.

        The public offset_cumsum wrapper always performs an all_gather first,
        which requires fabric context even when the mesh has a single device.
        For one device, the gathered histogram is just the local histogram, the
        local prefix component is zero, and only the tile-aligned expert-region
        prefix sum is needed. When host routing produced the indices, prefer its
        histogram to avoid unsigned integer readback on torch builds without
        uint16/uint32 support.
        """
        if host_expert_histogram is None:
            hist = ttnn.to_torch(expert_histograms).reshape(-1).to(torch.int64)
        else:
            hist = host_expert_histogram.reshape(-1).to(torch.int64)
        n_routed_experts = hist.numel()
        assert (
            n_routed_experts % self.experts_per_chip == 0
        ), f"n_routed_experts ({n_routed_experts}) must be divisible by experts_per_chip ({self.experts_per_chip})"

        totals = hist.clone()
        expert_region = torch.zeros_like(totals)
        for group_start in range(0, n_routed_experts, self.experts_per_chip):
            prefix = 0
            for local_expert in range(self.experts_per_chip):
                idx = group_start + local_expert
                expert_region[idx] = prefix
                prefix += int((totals[idx].item() + ttnn.TILE_SIZE - 1) // ttnn.TILE_SIZE * ttnn.TILE_SIZE)

        offsets = expert_region.clone()
        self.last_host_dispatch_offsets = offsets.clone()
        self.last_host_total_counts = totals.clone()
        self.last_host_expert_region_offsets = expert_region.clone()
        mapper = ttnn.ReplicateTensorToMesh(self.mesh_device)

        def to_device(values: torch.Tensor) -> ttnn.Tensor:
            return ttnn.from_torch(
                values.reshape(1, n_routed_experts).to(torch.int32),
        ...
        return to_device(offsets), to_device(totals), to_device(expert_region)
    ...
    def forward(
        ...
        self.last_host_dispatch_offsets = None
        self.last_host_total_counts = None
        ...
        if self.mesh_device.get_num_devices() == 1:
            (
                ...
                total_counts_per_expert,
                expert_region_offsets,
            ) = self._single_device_offset_cumsum(expert_histograms, host_expert_histogram=host_expert_histogram)
        else:
            (
                global_dispatch_offsets,
                total_counts_per_expert,
