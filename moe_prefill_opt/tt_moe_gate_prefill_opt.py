...
from moe_prefill_opt.tt_moe_routing_setup_opt import TtMoERoutingSetup


def sample_frequency_routing(
    frequency: torch.Tensor,
    num_tokens: int,
    topk: int,
    seed: int,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Create top-k routing assignments whose aggregate counts match a frequency profile.

    The returned indices are shaped ``[num_tokens, topk]`` with no duplicate expert
    within a token. Counts are rounded from ``frequency * num_tokens * topk`` so the
    measured assignment percentages are close to the requested profile.
    """
    frequency = frequency.detach().float().flatten()
    if frequency.numel() < topk:
        raise ValueError(f"Need at least topk={topk} experts, got {frequency.numel()}")
    if torch.any(frequency < 0):
        raise ValueError("frequency must be non-negative")
    total_probability = frequency.sum()
    if total_probability <= 0:
        raise ValueError("frequency must contain at least one positive value")

    probability = frequency / total_probability
    total_assignments = num_tokens * topk
    target = probability * total_assignments
    cap = torch.full_like(target, float(num_tokens))
    capped_target = torch.minimum(target, cap)
    counts = torch.floor(capped_target).to(torch.int64)
    remainder = total_assignments - int(counts.sum().item())
    capped_experts = int((target > cap).sum().item())
    if capped_experts > 0:
        logger.warning(
            "Frequency profile exceeds the no-duplicate-per-token limit for "
            f"{capped_experts} expert(s); capping each expert at num_tokens={num_tokens} "
            "and redistributing remaining top-k slots."
        )

    while remainder > 0:
        available = counts < num_tokens
        if not bool(available.any()):
            raise ValueError(
                "Unable to place requested top-k assignments without duplicate experts per token: "
                f"total_assignments={total_assignments}, num_tokens={num_tokens}, topk={topk}"
            )
        residual = torch.where(available, target - counts.float(), torch.full_like(target, -float("inf")))
        order = torch.argsort(residual, descending=True)
        placed_this_round = 0
        for expert_id in order.tolist():
            if remainder == 0:
                break
            if counts[expert_id] >= num_tokens:
                continue
            counts[expert_id] += 1
            remainder -= 1
            placed_this_round += 1
        if placed_this_round == 0:
            raise RuntimeError("Frequency routing count redistribution made no progress")

    generator = torch.Generator(device="cpu")
    generator.manual_seed(seed)
    indices = torch.full((num_tokens, topk), -1, dtype=torch.int64)
    fill = torch.zeros(num_tokens, dtype=torch.int64)

    expert_order = torch.argsort(counts, descending=True)
    for expert_id in expert_order.tolist():
        ...
        placed = 0
        noise = torch.rand(num_tokens, generator=generator)
        token_order = sorted(range(num_tokens), key=lambda token_idx: (int(fill[token_idx].item()), float(noise[token_idx])))
        for token_idx in token_order:
            if placed == count:
                break
            slot = int(fill[token_idx].item())
            if slot >= topk:
                continue
            indices[token_idx, slot] = expert_id
            fill[token_idx] += 1
            placed += 1
        ...
        if placed != count:
            raise RuntimeError(f"Unable to place all assignments for expert {expert_id}: {placed}/{count}")

    if torch.any(indices < 0):
        empty = torch.nonzero(indices < 0, as_tuple=False)
        raise RuntimeError(f"Frequency routing left {empty.shape[0]} unfilled top-k slots")

    shuffled_indices = indices.clone()
    for token_idx in range(num_tokens):
        order = torch.randperm(topk, generator=generator)
        shuffled_indices[token_idx] = indices[token_idx, order]

    selected_probs = probability[shuffled_indices]
    scores = selected_probs / selected_probs.sum(dim=-1, keepdim=True).clamp_min(1e-12)
    return shuffled_indices.to(torch.int64), scores.to(torch.bfloat16)
...
class TtMoEGatePrefill(LightweightModule):
    ...
    def __init__(
        ...
        self.last_host_expert_counts: torch.Tensor | None = None
        self.last_host_expert_offsets: torch.Tensor | None = None
        self.last_host_expert_region_offsets: torch.Tensor | None = None
        self.routing_frequency = None
        if routing_frequency is not None:
            frequency = routing_frequency.detach().float().flatten()
            if frequency.numel() != config.n_routed_experts:
                raise ValueError(
                    f"routing_frequency has {frequency.numel()} entries, expected {config.n_routed_experts}"
                )
            if torch.any(frequency < 0):
                raise ValueError("routing_frequency must be non-negative")
            total = frequency.sum()
            if total <= 0:
                raise ValueError("routing_frequency must contain at least one positive value")
            self.routing_frequency = frequency / total
            logger.info(
                "Using host frequency routing profile: "
                f"experts={config.n_routed_experts}, topk={config.n_activated_experts}"
            )
        ...
        # Reference model for host grouped-gate paths. Frequency-driven opt runs
        # never call grouped_gate_golden, so avoid importing transformers there.
        self.reference_model = None
        if fallback_mode in (GateComputeMode.HOST_GROUPED_GATE, GateComputeMode.HOST_ALL) and self.routing_frequency is None:
    ...
    def _host_indices_to_device(self, host_indices: torch.Tensor) -> ttnn.Tensor:
        if not hasattr(torch, "uint16"):
            ...
            if mesh_shape != (1, 1):
                raise RuntimeError(
                    "HOST_ALL synthetic routing needs torch.uint16 for multi-device sharded upload; "
                    "this torch build does not provide it. Use a torch build with uint16 support "
                    "or run the opt harness on a 1x1 mesh."
                )
            flat_indices = host_indices.contiguous().view(-1).to(torch.int32).tolist()
            return ttnn.from_buffer(
                flat_indices,
                list(host_indices.shape),
                ttnn.uint16,
                self.mesh_device,
                layout=ttnn.ROW_MAJOR_LAYOUT,
                memory_config=ttnn.DRAM_MEMORY_CONFIG,
    ...
    def _host_frequency_logits(self, num_tokens: int) -> torch.Tensor:
        """Create lightweight host logits matching the configured expert frequency profile."""
        assert self.routing_frequency is not None
        logits = torch.log(self.routing_frequency.clamp_min(1e-12))
        return logits.unsqueeze(0).expand(num_tokens, -1).to(torch.bfloat16).contiguous()

    def _host_frequency_gate(self, num_tokens: int) -> tuple[torch.Tensor, torch.Tensor]:
        """Create per-token top-k experts from the opt frequency profile."""
        assert self.routing_frequency is not None
        return sample_frequency_routing(
            self.routing_frequency,
            num_tokens=num_tokens,
            topk=self.config.n_activated_experts,
            seed=self.routing_seed + num_tokens * 1009 + self.config.n_activated_experts,
        )
    ...
    def forward(self, x: ttnn.Tensor) -> tuple[ttnn.Tensor, ttnn.Tensor, ttnn.Tensor, ttnn.Tensor, ttnn.Tensor]:
        ...
        host_indices_for_counts = None
        self.last_host_expert_counts = None
        self.last_host_expert_offsets = None
        self.last_host_expert_region_offsets = None
        ...
        elif self.routing_frequency is not None:
            host_logits = self._host_frequency_logits(int(x.shape[0]))
        ...
        if mode == GateComputeMode.DEVICE:
            ...
            if self.routing_frequency is not None:
                host_indices, host_scores = self._host_frequency_gate(host_logits.shape[0])
            else:
                host_indices, host_scores = self._host_grouped_gate(host_logits)
            host_indices_for_counts = host_indices
        ...
        host_expert_histogram = None
        if host_indices_for_counts is not None:
            host_expert_histogram = torch.bincount(
                host_indices_for_counts.reshape(-1).to(torch.int64),
        ...
        dispatch_offsets, total_counts_per_expert, expert_region_offsets, _ = self.routing_setup(
            ...
            host_expert_histogram=host_expert_histogram,
        )
        self.last_host_expert_counts = self.routing_setup.last_host_total_counts
        self.last_host_expert_offsets = self.routing_setup.last_host_dispatch_offsets
        self.last_host_expert_region_offsets = self.routing_setup.last_host_expert_region_offsets
        signpost(header="moe_gate_calculate_dispatch_offsets")
